...
from ..repo import add_evidence, emit_change, start_run, finish_run
...
def _study_belongs_to_company(study: dict[str, Any], sponsor_aliases: list[str]) -> bool:
    ...
    for cn in collab_names:
        if _company_alias_hit(cn, sponsor_aliases):
            return True
    return False
...
def _get_asset_alias_terms(session: Session, company_id: str) -> list[str]:
    ...
    aliases = [r[0] for r in session.execute(stmt).all()]
    ...
    for a in aliases:
        a = a.strip()
        if len(a) < settings.min_alias_len_for_trial_search:
            continue
        low = a.lower()
        if "undisclosed" in low:
            continue
        # avoid querying extremely generic components
        if low in {"other", "others", "unknown"}:
            continue
        # avoid parenthetical composite labels
        if "(" in a and ")" in a:
            continue
        k = norm_text(a)
        if k in seen:
    ...
    # prefer shorter aliases first (less query load)
    out.sort(key=len)
    return out


def _build_alias_index(session: Session, company_id: str) -> dict[str, int]:
    stmt = select(models.AssetAlias.alias_norm, models.AssetAlias.asset_id).join(models.Asset).where(models.Asset.company_id == company_id)
    idx: dict[str, int] = {}
        ...
        for alias in alias_terms:
            ...
            while True:
                ...
                for study in studies:
                    ...
                    h, p, meta = store_json(company_id, "ctg_study_json", f"{CTG_STUDIES_ENDPOINT}?nct={nct}", study, meta={"query_intr": alias})
                    ev = add_evidence(session, company_id, "ctg_study_json", f"{CTG_STUDIES_ENDPOINT}?nct={nct}", h, str(p), meta=meta)

                    # upsert trial
                    existing = session.execute(select(models.Trial).where(models.Trial.company_id == company_id, models.Trial.nct_id == nct)).scalar_one_or_none()
                    if existing is None:
                        tr = models.Trial(
                        ...
                        if new_status and old_status != new_status:
                            ...
                            status_changed += 1
                            emit_change(session, company_id, "trial_status_changed", {"nct_id": nct, "from": old_status, "to": new_status}, evidence_id=ev.id, trial_id=tr.id)
                    ...
                    # replace interventions and conditions
