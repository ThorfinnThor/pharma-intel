...
from ..sanitize import sanitize_asset_label, sanitize_alias, is_plausible_asset_label, sanitize_indication_text
...
def ingest_immatics_pipeline(session: Session, company_id: str = "immatics", curated_file: str | None = "configs/immatics_curated_assets.yaml") -> int:
    ...
    for ca in curated_assets:
        label = ca["name"]
        label = sanitize_asset_label(label) or label
        ...
        if not is_plausible_asset_label(canonical):
            logger.warning("Skipping implausible Immatics asset label: {}", canonical)
            continue
        ...
        asset = upsert_asset(
            ...
            company_id,
            canonical,
            modality=ca.get("modality"),
            target=ca.get("target"),
            is_disclosed=ca.get("is_disclosed", True),
        )
        ...
        for ind in ca.get("indications", []):
            indications.append({
                "indication": sanitize_indication_text(ind["indication"]),
    ...
    for token in page_assets:
        ...
        if asset:
            ensure_alias(session, asset.id, tok)
